##########################################################
# Title: XGBoost_nrounds_500_max_depth _3_eta_0_1_gamma_1_colsample_bytree_0_6_min_child_weight_10_subsample_0_7
##########################################################
...
Xgboost_tree <- train(Pobre ~ Cuartos + CuartosDormir + TenenciaVivienda + CuotaAmortizacion + ArriendoEst + ArriendoEfec + Npersonas + NpersonasUG + 
                        ...
                        H_Head_Posicion + 
                        H_Head_AuxAlimMesPasado + 
                        H_Head_AlimentosMesPasado + H_Head_ViviendaMesPasado + 
                        H_Head_TranspEmpresa + H_Head_Prima + H_Head_PrimaNavidad + 
                        H_Head_HorasT + 
                        H_Head_TamañoEmp + H_Head_CotPen +  H_Head_HorasTOtro + 
                        H_Head_PosicionOtro + 
                        H_Head_DispCambioEmpl + H_Head_BusqTrabajo +
                        H_Head_IngTrabDesocu + H_Head_IngPension +
                        H_Head_IngPInterior +
                        nMujeres + 
                        nAfiliados + nSubsidiado + maxEducLevel + nTrabajadores + maxTiempoTr + nEmpleado + 
                        nJefe + nCuentaPropia + nMenorRango + nIngHoraT + nBonoMesPasado + 
                        nAuxTransMesPasado +
                        nAlimentosMesPasado + nPrima + 
                        nPrimaNavidad + nViaticos + nTiempoCompleto + maxTamañoEmp + 
                        nCotPen + nOtroEmpleo + nDisponibilidad +
                        nBusqTrabajo + nIngTrabDesocu + nIngArrPens + nIngPension + nIngPAoDI + 
                        nIngInstDivCes + nIngPInterior + nIngPExterior + nIngInst + nOcupado,
                      data = train, 
                      method = "xgbTree", 
                      trControl = ctrl,
...
#Getting the graph of the tree

xgb_boost_model <- Xgboost_tree$finalModel
xgb.plot.tree(model = xgb_boost_model, trees = 0)

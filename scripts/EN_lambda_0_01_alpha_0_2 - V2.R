...
) |> select(id, Orden, mujer, H_Head, EducLevel, ocupado, CotPen, Prima, RegSalud, Edad, HorasT)
...
train_personas_nivel_hogar <- train_personas |>
  group_by(id) |> summarize (
    ...
    nSubsidiado = sum(RegSalud==3, na.rm=T),
  )

train_personas_hogar <- train_personas |> filter(H_Head==1) |>
  select(id, mujer, EducLevel, ocupado, CotPen, Prima, RegSalud, Edad, HorasT) |>
  rename(H_Head_mujer=mujer, H_Head_Edu=EducLevel, H_Head_ocupado = ocupado,
         H_Head_cotpen=CotPen, H_Head_prima=Prima, H_Head_RedSalud=RegSalud,
         H_Head_Edad=Edad, H_Head_HorasT=HorasT) |> 

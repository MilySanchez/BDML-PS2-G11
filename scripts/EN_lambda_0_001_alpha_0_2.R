# Este modelo integra las anteriores variables de Pensión, Prima y Arriendo
# como en el modelo 3, con la diferencia que ya no se toma el máximo nivel
# de eduación como variable, sino que se evalúa si la persona alcanazó estudios
# universitarios.
# Puntuación 0.5824
# Clear workspace
...
train_personas_nivel_hogar <- train_personas |>
  group_by(id) |> summarize (
    ...
    nUni=sum(Uni,na.rm=TRUE),
    ncotpen=sum(CotPen, na.rm=TRUE),
    nprima=sum(Prima, na.rm=TRUE)
  )

train_personas_hogar <- train_personas |> filter(H_Head==1) |>
  select(id, mujer, Uni, ocupado, CotPen, Prima) |>
  rename(H_Head_mujer=mujer, H_Head_Uni=Uni, H_Head_ocupado = ocupado,

...
train_personas<- train_personas %>% mutate(
  ...
  dineroExtra = ifelse(is.na(P7505),0,ifelse(P7505==1,1,0))
) |> select(id, Orden, mujer, H_Head, EducLevel, ocupado, CotPen, Prima, dineroExtra)
...
train_personas_nivel_hogar <- train_personas |>
  group_by(id) |> summarize (
    ...
    ndineroExtra=sum(dineroExtra, na.rm=T)
  )

train_personas_hogar <- train_personas |> filter(H_Head==1) |>
  select(id, mujer, EducLevel, ocupado, CotPen, Prima, dineroExtra) |>
  rename(H_Head_mujer=mujer, H_Head_Edu=EducLevel, H_Head_ocupado = ocupado,
         H_Head_cotpen=CotPen, H_Head_prima=Prima, H_Head_dineroExtra=dineroExtra) |> 

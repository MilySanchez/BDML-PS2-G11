##########################################################
# Title: XGBoost_nrounds_500_max_depth _3_eta_0_1_gamma_1_colsample_bytree_0_8_min_child_weight_10_subsample_0_7 - V2
##########################################################
...
Xgboost_tree <- train(Pobre ~ Cuartos + CuartosDormir + TenenciaVivienda + ArriendoEst + ArriendoEfec + Npersonas + NpersonasUG + 
                        Lindigencia + Lpobreza +Depto + arriendo + H_Head_Mujer + H_Head_EducLevel +  H_Head_TiempoTr + 
                        H_Head_Posicion + H_Head_ViviendaMesPasado + H_Head_HorasT + +H_Head_IngPension +
                        nAfiliados + nSubsidiado + maxEducLevel + nTrabajadores + maxTiempoTr + nEmpleado + 
                        nAlimentosMesPasado + nPrima + nTiempoCompleto + maxTamañoEmp + 
                        nCotPen + nBusqTrabajo + nIngTrabDesocu + nIngArrPens + nIngPension +
                        nIngPInterior + nIngPExterior +  nOcupado,
                      data = train, 
                      method = "xgbTree", 
                      trControl = ctrl,

...
# lo que pagasen. Puntuación 0.6123
# Clear workspace
...
) |> select(id, Orden, mujer, H_Head, EducLevel, ocupado, CotPen, Prima, RegSalud)
...
train_personas_hogar <- train_personas |> filter(H_Head==1) |>
  select(id, mujer, EducLevel, ocupado, CotPen, Prima, RegSalud) |>
